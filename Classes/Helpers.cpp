...
Vec2 helpers::setDesignPosEx(cocos2d::Node* node, int x, int y)
{
	...
	float rx = static_cast<float>(x)+s.width / 2.0 - dw / 2.0;
	float ry = (dh - static_cast<float>(y)-1) - s.height / 2.0 - dh / 2.0;
	node->setPosition(Vec2(rx, ry));
    return Vec2(rx, ry);
}
