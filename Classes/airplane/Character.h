...
public:
    ...
    // передается пол, что бы подписаться на события связанные с ним
...
/* класс от которого наследуются поведенческие классы, например пойти в туалет,
	и он будет полностью управлять объектом пока объект не дойдет до туалета
	и не сделает в нем все дела, после чего в зависимости от состояния, влючается 
	новый поведенческий класс

    класс может передать колбэк на событие, когда объет дойдет то таргета, узнать 
	косается ли объект пола и прк задавать анимацию
 */
class CharacrerBehaviour {
